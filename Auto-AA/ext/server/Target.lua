...
function Target:getVelocity()
    if self.vehicle ~= nil then
        local v = PhysicsEntity(self.vehicle).velocity
        local s = math.sqrt(v.x^2 + v.y^2 + v.z^2)
        return s * 3.6 --convert to km/h
    end
end
...
function Target:getDistance(botPos)
    ...
    local targetPos = self:getInterceptingPosition(botPos)
    local distance = math.sqrt((targetPos.x - botPos.x)^2 + (targetPos.y - botPos.y)^2 + (targetPos.z - botPos.z)^2)
    return distance
end

function Target:isInRange(botPos)
    if botPos == nil then
        return false
    end

    local targetPos = self:getInterceptingPosition(botPos)
    local distance = math.sqrt((targetPos.x - botPos.x)^2 + (targetPos.z - botPos.z)^2)
